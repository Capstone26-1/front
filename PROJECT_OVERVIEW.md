...
[백엔드 - deployment-production-91c7.up.railway.app]
  ...
  └─ server/claude.js  ← Claude API 호출 + Tool 실행
      ...
      └─ MCP TOOLS (동적 선택 — 상황에 따라 자동 결정)
          ...
          └─ validate_transit_route  → Anthropic haiku 서브호출로 막차 종착역 검증
                                       도달 불가 시 종착역부터 목적지까지 Tmap 재탐색
                                       + latestSafeDeparture (지하철 가능 마지노선 시각)
```
...
transit_disruption_tool  ← 항상 호출 (실시간 막차 여부·운행 지연 확인)
    │
    ▼
validate_transit_route   ← SUBWAY leg 있으면 필수 호출
    │   haiku 서브호출로 leg마다 막차 종착역 검증
    │   → feasible: true  → 경로 정상
    │   → feasible: false → lastReachableStation + latestSafeDeparture 반환
    │                        Kakao + Tmap으로 종착역부터 목적지 재탐색
    │
    ├─ 정상 운행 ────────────────────── 위험도 판정
    │
    └─ 막차 도달 불가 or (riskScore ≥ 70 + 22시 이후)
         │
         ▼
    taxi_fare_tool       ← 마지막 도달 가능 역 → 목적지 요금 계산
         │
         ▼
    위험도 판정 (riskScore 0~100)
    verdict: 안전 / 주의 / 위험 / 매우 위험
    + 택시 대안 카드 (사당역→목적지)
    + latestSafeDeparture 추천 ("HH:MM 이전 출발 시 지하철 가능")
```
...
| F. 막차 종착역 버그 | "인덕원역에서 돌곶이역 23시 40분 출발" | + `validate_transit_route` (haiku) → 사당 종착 감지 → 택시(사당→돌곶이) + "23:10 이전 출발 추천" |

시나리오 A는 Tool 4개, B~D는 5개, E~F는 7개 — 동적 선택이 명확히 보이는 구조입니다.
...
│       └── subwayValidator.js ← haiku 서브호출로 막차 종착역 검증 (validateSubwayLeg)
│
├── Features/               ← 기능별 구현 문서
│   ├── validate_transit_route.md  ← 막차 종착역 검증 + 택시 대안
│   └── recommendation.md          ← 출발 시각 앞당기기 추천
├── Bugs/                   ← 버그 분석 및 해결 기록
